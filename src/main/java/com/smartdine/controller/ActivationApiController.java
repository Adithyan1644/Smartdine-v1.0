...
public class ActivationApiController {
        ...
        try {
            ...
            for (java.io.File dir : dirs) {
                if (dir.exists() && dir.isDirectory()) {
                    java.io.File[] files = dir
                            .listFiles((d, name) -> name.startsWith("activation-") && name.endsWith(".json"));
                    if (files != null) {
    ...
    @RequestMapping(value = "/keep-only", method = { RequestMethod.GET, RequestMethod.POST })
    public ResponseEntity<?> keepOnlyAccount(@RequestParam String code) {
        try {
            String targetCode = code.trim();
            var targetRestOpt = restaurantRepository.findBySyncCodeAndIsDeletedFalse(targetCode)
                .or(() -> restaurantRepository.findByBillerSyncCode(targetCode));

            if (targetRestOpt.isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of("success", false, "error", "Target sync code " + targetCode + " not found in database. Cannot purge others."));
            }

            com.smartdine.coreheart.Restaurant targetRest = targetRestOpt.get();
            java.util.UUID keepId = targetRest.getRestaurantId() != null ? targetRest.getRestaurantId() : targetRest.getId();

            // Purge all other restaurants and users except target
            int deletedRest = jdbcTemplate.update("DELETE FROM restaurants WHERE restaurant_id != ? AND id != ?", keepId, keepId);
            jdbcTemplate.update("DELETE FROM app_users WHERE restaurant_id != ?", keepId);

            return ResponseEntity.ok(Map.of(
                "success", true,
                "retainedAccount", targetRest.getName(),
                "syncCode", targetCode,
                "deletedCount", deletedRest,
                "message", "Retained account " + targetRest.getName() + " (" + targetCode + ") and purged all other accounts."
            ));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("error", e.getMessage()));
        }
    ...
    public ResponseEntity<?> getPingStatus(@RequestParam(required = false) String code) {
        ...
        return ResponseEntity.ok(Map.of("active", active, "lastPing", lastPing != null ? lastPing : 0, "status",
        ...
        if (files != null) {
            ...
            for (java.io.File file : files) {
                try {
                    ...
                    String rName = map.get("restaurantName") != null
                            ? map.get("restaurantName").toString().trim().toLowerCase()
                            : "";
                    String rPhone = map.get("ownerPhone") != null
                            ? map.get("ownerPhone").toString().replaceAll("\\D", "")
                            : "";
                    String rEmail = map.get("ownerEmail") != null
                            ? map.get("ownerEmail").toString().trim().toLowerCase()
                            : "";

                    if (!cleanedName.isEmpty() && rName.equals(cleanedName)) {
                        return ResponseEntity
                                .ok(Map.of("available", false, "reason", "Restaurant Name '" + restaurantName
                                        + "' is already registered. Please choose a different restaurant name."));
                    }
                    if (!cleanedPhone.isEmpty() && !rPhone.isEmpty() && rPhone.contains(cleanedPhone)) {
                        return ResponseEntity.ok(Map.of("available", false, "reason", "Mobile Number '" + phone
                                + "' is already registered. Please use a different mobile number."));
                    }
                    if (!cleanedEmail.isEmpty() && !rEmail.isEmpty() && rEmail.equals(cleanedEmail)) {
                        return ResponseEntity.ok(Map.of("available", false, "reason",
    ...
    public ResponseEntity<?> saveConfig(@RequestBody Map<String, Object> request) {
        try {
            ...
            if (restName == null && request.get("profile") instanceof Map) {
                ...
                if (pName != null)
                    restName = pName.toString();
            }
            if (restName == null || restName.isEmpty())
                restName = "SmartDine Restaurant";
            ...
            java.util.List<Map<String, Object>> incomingTables = (java.util.List<Map<String, Object>>) request
                    .get("tables");
            java.util.List<Map<String, Object>> incomingMenuItems = (java.util.List<Map<String, Object>>) request
                    .get("menuItems");
            java.util.List<Map<String, Object>> incomingWaiters = (java.util.List<Map<String, Object>>) request
                    .get("waiters");
            ...
            if (incomingTables != null) {
                for (Map<String, Object> t : incomingTables) {
                    ...
                    tMap.put("capacity",
                            t.get("capacity") != null ? Integer.parseInt(t.get("capacity").toString()) : 4);
            ...
            if (incomingMenuItems != null) {
                ...
                for (Map<String, Object> item : incomingMenuItems) {
                    ...
                    String baseCode = item.get("code") != null && !item.get("code").toString().trim().isEmpty()
                            ...
                            : (name.length() >= 3 ? name.substring(0, 3).toUpperCase().replaceAll("[^A-Z0-9]", "")
                                    : "ITM");
                    if (baseCode.isEmpty())
                        baseCode = "ITM";
                    ...
                    String category = item.get("categoryName") != null ? item.get("categoryName").toString()
                            : (item.get("category") != null ? item.get("category").toString() : "General");
            ...
            if (incomingWaiters != null) {
                for (Map<String, Object> w : incomingWaiters) {
                    ...
                    wMap.put("pin", w.get("pin") != null ? w.get("pin").toString()
                            : (w.get("code") != null ? w.get("code").toString() : ""));
                    wMap.put("status", w.get("status") != null ? w.get("status").toString() : "Active");
                    mappedWaiters.add(wMap);
                }
            }

            java.util.List<Map<String, Object>> incomingAddons = (java.util.List<Map<String, Object>>) request
                    .get("addons");
            ...
            if (incomingAddons != null && !incomingAddons.isEmpty()) {
                for (Map<String, Object> addonMap : incomingAddons) {
                    if (addonMap.get("name") != null) {
                        double price = addonMap.get("price") != null
                                ? Double.parseDouble(addonMap.get("price").toString())
                                : 0.0;
                ...
                try {
                    ...
                    java.util.List<com.smartdine.coreheart.AddonItem> liveAddons = addonItemRepository
                            .findByRestaurantId(rUuid);
            ...
            gatewayPayload.put("cgstRate",
                    request.get("cgstRate") != null ? new java.math.BigDecimal(request.get("cgstRate").toString())
                            : new java.math.BigDecimal("2.50"));
            gatewayPayload.put("sgstRate",
                    request.get("sgstRate") != null ? new java.math.BigDecimal(request.get("sgstRate").toString())
                            : new java.math.BigDecimal("2.50"));
            ...
            gatewayPayload.put("deliveryChargeEnabled",
                    request.get("deliveryChargeEnabled") != null ? request.get("deliveryChargeEnabled") : false);
            gatewayPayload.put("defaultDeliveryFee",
                    request.get("defaultDeliveryFee") != null
                            ? Double.parseDouble(request.get("defaultDeliveryFee").toString())
                            : 0.0);
            gatewayPayload.put("packingChargeEnabled",
                    request.get("packingChargeEnabled") != null ? request.get("packingChargeEnabled") : false);
            gatewayPayload.put("defaultPackingFee",
                    request.get("defaultPackingFee") != null
                            ? Double.parseDouble(request.get("defaultPackingFee").toString())
                            : 0.0);
            ...
            try {
                mapper.writeValue(new java.io.File("core-heart/" + fileName), gatewayPayload);
            } catch (Exception ignored) {
            }
            try {
                mapper.writeValue(new java.io.File("core-heart/core-heart/" + fileName), gatewayPayload);
            } catch (Exception ignored) {
            }
            try {
                mapper.writeValue(new java.io.File("activation-data.json"), gatewayPayload);
            } catch (Exception ignored) {
            }

            // Immediately sync payload directly into the active PostgreSQL database tables
            // (menu_items, dining_tables, etc.)
            try {
                activationService.syncCloudConfiguration(gatewayPayload);
                System.out.println(
                        "☁️ Live Sync: Web configuration successfully saved directly into PostgreSQL database.");
            } catch (Exception e) {
    ...
    public ResponseEntity<?> getConfig(@RequestParam(required = false) String code) {
        try {
            ...
            java.util.List<com.smartdine.coreheart.DiningTable> tables = tableRepository
            ...
            java.util.List<com.smartdine.coreheart.MenuItem> menuItems = menuRepository
            ...
            java.util.List<com.smartdine.coreheart.Category> dbCats = categoryRepository
                    .findByRestaurantId(restaurantId);
            for (var c : dbCats) {
            ...
            // Fetch live waiters
            java.util.List<com.smartdine.coreheart.AppUser> dbWaiters = userRepository
                    .findByRestaurantIdAndRoleAndIsActiveTrue(restaurantId, com.smartdine.coreheart.UserRole.WAITER);
            java.util.List<Map<String, Object>> mappedWaiters = new java.util.ArrayList<>();
            for (var w : dbWaiters) {
            ...
            // Fetch live addons
            java.util.List<com.smartdine.coreheart.AddonItem> dbAddons = addonItemRepository
                    .findByRestaurantId(restaurantId);
    ...
    public ResponseEntity<?> getAnalytics(@RequestParam(required = false) String code) {
        ...
        for (com.smartdine.coreheart.Order o : allOrders) {
            ...
            if (started != null && !started.isBefore(todayStart)) {
                ...
                if (isNonCancelled)
                    yesterdaySales = yesterdaySales.add(total);
            }

            if (!started.isBefore(weekStart)) {
                if (isNonCancelled)
                    weeklySales = weeklySales.add(total);
            }

            if (!started.isBefore(monthStart)) {
                if (isNonCancelled)
                    monthlySales = monthlySales.add(total);
            }
        }
        ...
        java.util.List<Map<String, Object>> distribution = java.util.List.of(
                Map.of("name", "Dine-In", "value", todayDineInSales.doubleValue(), "pct",
                        totalSalesD > 0 ? Math.round((todayDineInSales.doubleValue() * 100) / totalSalesD) : 0, "color",
                        "#0B6B50"),
                Map.of("name", "Takeaway", "value", todayTakeawaySales.doubleValue(), "pct",
                        totalSalesD > 0 ? Math.round((todayTakeawaySales.doubleValue() * 100) / totalSalesD) : 0,
                        "color", "#F59E0B"),
                Map.of("name", "Online", "value", todayDeliverySales.doubleValue(), "pct",
                        totalSalesD > 0 ? Math.round((todayDeliverySales.doubleValue() * 100) / totalSalesD) : 0,
                        "color", "#3B82F6"));

        // Payment methods lists for Sales screen
        java.util.List<Map<String, Object>> paymentMethods = java.util.List.of(
                Map.of("method", "UPI", "amount", todayUpiSales.doubleValue(), "pct",
                        totalSalesD > 0 ? Math.round((todayUpiSales.doubleValue() * 100) / totalSalesD) : 0, "color",
                        "#0B6B50"),
                Map.of("method", "Cash", "amount", todayCashSales.doubleValue(), "pct",
                        totalSalesD > 0 ? Math.round((todayCashSales.doubleValue() * 100) / totalSalesD) : 0, "color",
                        "#F59E0B"),
                Map.of("method", "Card", "amount", todayCardSales.doubleValue(), "pct",
                        totalSalesD > 0 ? Math.round((todayCardSales.doubleValue() * 100) / totalSalesD) : 0, "color",
                        "#3B82F6"));
        ...
        int maxSlots = Math.max(1,
                Math.max(slot8to10, Math.max(slot12to2, Math.max(slot4to6, Math.max(slot7to9, slot9to11)))));
        java.util.List<Map<String, Object>> peakHours = java.util.List.of(
                Map.of("slot", "8 AM – 10 AM", "label", "Breakfast Rush", "orders", slot8to10, "pct",
                        Math.round((slot8to10 * 100.0) / maxSlots)),
                Map.of("slot", "12 PM – 2 PM", "label", "Lunch Rush", "orders", slot12to2, "pct",
                        Math.round((slot12to2 * 100.0) / maxSlots)),
                Map.of("slot", "4 PM – 6 PM", "label", "Evening Snacks", "orders", slot4to6, "pct",
                        Math.round((slot4to6 * 100.0) / maxSlots)),
                Map.of("slot", "7 PM – 9 PM", "label", "Dinner Rush", "orders", slot7to9, "pct",
                        Math.round((slot7to9 * 100.0) / maxSlots)),
                Map.of("slot", "9 PM – 11 PM", "label", "Late Dinner", "orders", slot9to11, "pct",
                        Math.round((slot9to11 * 100.0) / maxSlots)));
        ...
        if (todayOrdersCount > 0) {
            ...
            pulse.add(
                    "Average order value stands at ₹" + Math.round(todaySales.doubleValue() / todayOrdersCount) + ".");
        ...
        double compPct = yesterdaySales.doubleValue() > 0
                ? Math.round((compDiff.doubleValue() * 100.0) / yesterdaySales.doubleValue())
                : 0.0;
        java.util.List<Map<String, Object>> comparison = java.util.List.of(
                Map.of("label", "Today vs Yesterday", "diff", compDiff.doubleValue(), "pct", compPct, "positive",
                        compDiff.doubleValue() >= 0),
                Map.of("label", "This Week vs Last Week", "diff", weeklySales.doubleValue(), "pct", 0.0, "positive",
                        true),
                Map.of("label", "This Month vs Last Month", "diff", monthlySales.doubleValue(), "pct", 0.0, "positive",
                        true));

        // Insights for Sales screen
        java.util.List<String> salesInsights = new java.util.ArrayList<>();
        if (todayOrdersCount > 0) {
            salesInsights.add("Sales today reached ₹" + todaySales.doubleValue() + ", driven by " + todayOrdersCount
                    + " covers.");
        ...
        for (java.util.Map.Entry<String, int[]> entry : dishStats.entrySet()) {
            ...
            if (nameLower.contains("naan") || nameLower.contains("roti") || nameLower.contains("tandoor")
                    || nameLower.contains("kebab") || nameLower.contains("tikka")) {
                station = "Tandoor & Grill";
                cat = "Starters & Breads";
                basePrep = 8;
                tandoorItems += totalOrd;
            } else if (nameLower.contains("soda") || nameLower.contains("juice") || nameLower.contains("tea")
                    || nameLower.contains("coffee") || nameLower.contains("drink") || nameLower.contains("water")
                    || nameLower.contains("bar") || nameLower.contains("lassi")) {
                station = "Bar & Beverages";
                cat = "Beverages";
                basePrep = 4;
                barItems += totalOrd;
            } else if (nameLower.contains("ice cream") || nameLower.contains("jamun") || nameLower.contains("sweet")
                    || nameLower.contains("kheer") || nameLower.contains("dessert") || nameLower.contains("halwa")) {
            ...
            int slaPct = totalOrd > 0
                    ? Math.max(0, Math.min(100, Math.round(((totalOrd - delCount) * 100f) / totalOrd)))
                    : 100;
            String status = delCount > 0 ? "Delayed"
                    : (calcAvg <= 5 ? "Fastest" : (calcAvg <= targetSla ? "On Time" : "Nearing SLA"));
        ...
        java.util.List<Map<String, Object>> realStationWorkload = java.util.List.of(
                Map.of("name", "Main Kitchen", "count", mainKitchenItems, "pct",
                        grandItemCount > 0 ? Math.round((mainKitchenItems * 100f) / grandItemCount) : 0, "color",
                        "#0B6B50"),
                Map.of("name", "Tandoor & Grill", "count", tandoorItems, "pct",
                        grandItemCount > 0 ? Math.round((tandoorItems * 100f) / grandItemCount) : 0, "color",
                        "#F59E0B"),
                Map.of("name", "Bar & Beverages", "count", barItems, "pct",
                        grandItemCount > 0 ? Math.round((barItems * 100f) / grandItemCount) : 0, "color", "#3B82F6"),
                Map.of("name", "Desserts & Pantry", "count", dessertItems, "pct",
                        grandItemCount > 0 ? Math.round((dessertItems * 100f) / grandItemCount) : 0, "color",
                        "#8B5CF6"));

        int overallSlaCompliance = totalKotsCount > 0
                ? Math.max(0, Math.min(100, Math.round(((totalKotsCount - delayedKotsCount) * 100f) / totalKotsCount)))
                : 100;
        ...
        overviewMap.put("insights", java.util.List.of(
                ...
                Map.of("priority", "Medium", "title", "Live Connection", "desc",
                        "POS terminal is active and broadcasting.")));
        ...
        response.put("sales", Map.of(
                "kpis", Map.of(
                        "today",
                        Map.of("value", todaySales.doubleValue(), "change", compPct, "positive",
                                compDiff.doubleValue() >= 0),
        ...
        response.put("kitchen", Map.of(
                ...
                "kitchenKpis", java.util.List.of(
                        Map.of("label", "Avg Prep Time", "value", todayOrdersCount > 0 ? "12 mins" : "—", "change", 0,
                                "positive", true),
                        Map.of("label", "Kitchen Load", "value", todayOrdersCount > 0 ? "Normal" : "Light", "change", 0,
                                "positive", true),
